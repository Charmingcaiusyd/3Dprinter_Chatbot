...
class MessageViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        queryset = Message.objects.filter(user=self.request.user).order_by(
            "-created_at"
        )
        conversationId = self.request.query_params.get("conversationId")
        if conversationId:
            queryset = queryset.filter(conversation_id=conversationId).order_by(
                "created_at"
            )
            return queryset
        return queryset
...
class EmbeddingDocumentViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        return EmbeddingDocument.objects.filter(user=self.request.user).order_by(
            "-created_at"
        )
...
def upload_conversations(request):
    ...
    try:
        ...
        logger.debug(e)
        return Response({"error": import_err_msg}, status=status.HTTP_400_BAD_REQUEST)
...
def conversation(request):
    ...
    def stream_content():
        try:
            if messages["renew"]:
                model_name = get_model_name()
                logger.warn("model_name = %s", model_name)
                openai_response = my_openai.ChatCompletion.create(
                    model=model_name,
    ...
    if messages.get("faiss_store", None) and not web_search_params:
        ...
        response = StreamingHttpResponse(
            stream_langchain(), content_type="text/event-stream"  # response generator
        )
    else:
        response = StreamingHttpResponse(
...
def create_message(
    ...
    message_obj = Message(
        ...
        embedding_message_doc=EmbeddingDocument.objects.get(pk=embedding_doc_id)
        if embedding_doc_id
        else None,
        messages=messages,
        tokens=tokens,
...
def build_messages(
    ...
    if conversation_id:
        ordered_messages = Message.objects.filter(
            conversation_id=conversation_id
        ).order_by("created_at")
    ...
    if database_mode:
        database_mode_sys_content = (
            "You are Q&A bot. A highly intelligent system that answers user questions based on the information provided by the user's local database. "
        ...
        try:
            cur_input = ordered_messages_list[-1]["message"]
            query_res = document_query.query(cur_input)
            logger.info(query_res)
            ordered_messages_list[-1][
                "message"
            ] = f"References:\n{query_res}\n\nUser's Question: {cur_input}"
            logger.info(ordered_messages_list[-1]["message"])
            system_messages = [{"role": "system", "content": database_mode_sys_content}]
        except Exception:
            logger.error("Get input error. Disable DATABASE_MODE")
    ...
    while current_token_count < max_token_count and len(ordered_messages_list) > 0:
        ...
        if web_search_params is not None and first_msg:
            search_results = web_search(
                SearchRequest(message["message"], ua=web_search_params["ua"]),
                num_results=5,
            )
            message_content = compile_prompt(
                search_results,
                message["message"],
        ...
        if message_type in [
            ...
            if message_type == Message.doc_context_message_type:
                ...
                if doc_id:
                    ...
                    if doc_obj:
                        logger.debug(
                            "get the document obj %s %s", doc_id, doc_obj.title
                        )
            ...
            new_token_count = num_tokens_from_messages(
                system_messages + messages + [new_message], model["name"]
            )
