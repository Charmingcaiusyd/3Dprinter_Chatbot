...
def upload_image(request, session_id):
    ...
    if request.method == 'POST' and request.FILES.get('image'):
        try:
            ...
            # 调用analyze_image函数处理图片
            yolo_result = analyze_image(image)

            # 存储YOLO输出到对话上下文
            chat_session = ChatSession.objects.get(id=session_id)
            chat_session.yolo_output = json.dumps(yolo_result['boxes_info'])
            chat_session.save()

            response_data.update({
                'url': file_url,
                'result_image_url': default_storage.url('result_image.jpg'),
