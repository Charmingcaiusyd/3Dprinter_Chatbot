...
const fetchReply = async (message) => {
  ...
  const data = Object.assign({}, currentModel.value, {
    ...
    databaseMode: databaseMode.value
  }, webSearchParams)
          ...
          <v-row>
            <v-col v-for="(message, index) in conversation.messages" :key="index" cols="12">
              <div class="d-flex align-center" :class="message.is_bot ? 'justify-start' : 'justify-end'">
                <MessageActions v-if="!message.is_bot" :message="message" :message-index="index" :use-prompt="usePrompt"
                  :delete-message="deleteMessage" :toggle-message="toggleMessage" />
  ...
  <v-footer app class="footer">
    <div class="px-md-16 w-100 d-flex flex-column">
      ...
      <v-toolbar density="comfortable" color="transparent">
        ...
        <v-spacer></v-spacer>
        <v-switch v-model="databaseMode" inline hide-details color="primary" :label="$t('databaseMode')"></v-switch>
        <div v-if="$settings.open_frugal_mode_control === 'True'" class="d-flex align-center">
          <v-switch v-model="frugalMode" inline hide-details color="primary" :label="$t('frugalMode')"></v-switch>
          <v-dialog transition="dialog-bottom-transition" width="auto">
            <template v-slot:activator="{ props }">
              <v-icon color="grey" v-bind="props" icon="help_outline" class="ml-3"></v-icon>
            </template>
